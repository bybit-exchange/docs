...
|**USDC Contract** |𐄂|𐄂 |✓<br/>(Unified Mode) |𐄂 |✓<br/>(Unified Mode)<br/>(To support Normal Mode soon) |
|**Inverse Contract** |𐄂|✓<br/>(Normal Mode)|𐄂 |✓<br/>(Normal Mode) |𐄂<br/>(To support Normal Mode soon) |
|**Option** |𐄂|𐄂 |✓<br/>(Unified Mode) |𐄂 | ✓<br/>(Unified Mode)<br/>(To support Normal Mode soon) |
...
API V5 offers clearer path definitions for improved clarity and reduced ambiguities. The new version divides the API
...
|:----- |-------|
|v5/market/|Candlestick, order book, ticker, platform transaction data, underlying financial rules, risk control rules|
...
* **V2 Rate Limit:** “User ID + Symbol + API End Point” limits the frequency of API requests by a fixed number of counts per minute
* **V3 Rate Limit:** “User ID + API End Point” limits the frequency of API requests by a fixed number of counts per second
* **V5 Rate Limit:** “User ID + API End Point” limits the frequency of API requests by a fixed number of counts per second
<br/>
...
More WebSocket data has been added to provide a range of order book data at different frequencies and **depths**. Push
frequency has been increased from 25 to 50. Please refer to the WebSocket interface description for more details.

The updated order book now checks for data continuity to ensure that local order book data is correct. A `Seq (sequence)`
...
|Product | Depth 1 | Depth 25 | Depth 50 | Depth 100 | Depth 200 |
|:----- |:----:|:----:|:----:|:----:|:----:|
|**Spot** | 10ms | - | 20ms | - | - |
|**Derivatives** | 10ms | - | 20ms | - | 100ms |
|**Option** | - | 20ms | - | 100ms | - |

### Increase in Post Only Orders
Under Derivatives, the submission of Post Only orders allows users to place more orders for each symbol — typically up
to five times the usual number. Users can use the [instrument-info](../v5/market/instrument) interface to obtain the maximum number of
orders that can be traded for each symbol.

### Uniform and Precise Data
Previous APIs returned data of type E2 and E8. API V3 and V5 return data in actual values, eliminating the need for
users to make conversion adjustments.
...
<table border="0.8">
    ...
    <tr>
        <td rowSpan="12">Position</td><td rowSpan="2">/v5/position/list</td><td>/unified/v3/private/position/list</td>
    </tr>
    <tr>
    ...
    <tr>
        <td rowSpan="2">Execution</td><td rowSpan="2">/v5/execution/list</td><td>/unified/v3/private/execution/list</td>
    </tr>
    <tr>
