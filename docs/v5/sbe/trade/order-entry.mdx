...
In version 2, we introduced a new boolean request parameter. However, for some programming languages, this may cause issues if the field is not explicitly included in the request payload.

For now, we recommend continuing to send requests using `version=1`. On the response side, you may still see `version=2`, as this is currently an internal testing version and has not been fully released yet.

The current `version=1` is fully stable and ready for production use.
:::

## Mainnet
The estimated available date is the beginning of June, 2026. Testnet URL: `wss://stream-testnet.bybits.org/v5/trade-sbe`
