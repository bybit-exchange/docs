...
|:----- |:-------|:-----|------ |
|coin |false |string |Coin name, e.g. `BTC`, `USDT`. If specified, total assets will be quoted in this coin|
...
|> origb |string |Original balance (base coin) |
|> quoteb |string |Original balance (quoted coin) |
|> items |array&lt;object&gt; |Sub-account type breakdown |
|>> type |string |Account type, e.g. `ACCOUNT_TYPE_FUND`, `ACCOUNT_TYPE_UNIFIED` |
|>> origb |string |Original balance |
|>> quoteb |string |Balance in quoted coin |
|>> stat |integer |Status |
