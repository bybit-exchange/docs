...
The `questionnaire` field is a JSON string used to carry Travel Rule compliance information in both the [Submit Deposit Originator Info](../deposit/submit-info#) and [Withdraw](/docs/v5/asset/withdraw) endpoints. It replaces the legacy `beneficiary` + `transactionPurpose` combination for new integrations.
