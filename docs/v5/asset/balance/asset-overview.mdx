...
:::info
This endpoint does not support any regional account, e.g., Bybit KZ, GE, EU, etc.
:::
...
|> categories |array |Sub-category breakdown. Returned for accounts that have product sub-categories (e.g. `Earn`, `TradingBot`, `CopyTrading`, `UnifiedTradingAccount`). For `UnifiedTradingAccount`, categories are `crypto` and `stocks` |
