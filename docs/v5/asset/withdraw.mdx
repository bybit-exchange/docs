...
|forceChain |false |integer |Whether or not to force an on-chain withdrawal<ul><li>`0`(default): If the address is parsed out to be an internal address, then internal transfer</li><li>`1`: Force the withdrawal to occur on-chain</li> </ul>|
