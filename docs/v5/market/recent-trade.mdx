...
|[symbol](../enum#symbol) |<div class="tooltip">false<span class="tooltiptext">Required for `spot`/`linear`/`inverse`. Optional for `option`.</span></div> |string |Symbol name, like `BTCUSDT`, uppercase only |
