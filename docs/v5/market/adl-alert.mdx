...
1. **Contract PnL drawdown ADL (based on the new grouped insurance pool mechanism, see examples 1 and 2)**  
   - **Trigger condition**:  
     ...
     - **pnlRatio**: drawdown ratio of the `symbol` in the last 8 hours  
       Formula: `pnlRatio` = (symbol Current PnL - symbol 8h Max PnL) / Insurance pool 8h max balance(`maxBalance`)  
       *Note: symbol Current PnL and symbol 8h Max PnL values are not provided by the API*.
