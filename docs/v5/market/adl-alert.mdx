...
1. **Contract PnL drawdown ADL (based on the new grouped insurance pool mechanism, see examples 1 and 2)**  
   - **Trigger condition**:  
     ...
     - **pnlRatio**: drawdown ratio of the symbol in the last 8 hours  
       ...
       *Note: 8h Max PnL and Current PnL values are not provided by the API*. Current PnL: realised PnL in the last 8 hours plus the current unrealised UPnL. =================== MAX PNL/CURRENT PNL DEFINITIONS NEED CHECKING
     - **Insurance pool 8h max balance**: the maximum balance of the grouped insurance pool in the last 8 hours   
...
1. **Example 1: Pool has no significant profit in the last 8 hours, then symbol loss triggers ADL**  
...
2. **Example 2: Pool has significant profit in the last 8 hours, but symbol loss still triggers ADL**  
