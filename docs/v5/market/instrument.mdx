...
|[symbolType](../enum#symboltype) |false |string |SymbolType:The region to which the trading pair belongs,only for`linear`,`inverse`|
|[status](../enum#status) |false |string |Symbol status filter <ul><li>By default returns only `Trading` symbols</li><li>Spot has `Trading` only</li><li>`linear` & `inverse`: when status=PreLaunch, it returns <a href="https://www.bybit.com/help-center/article/Introduction-to-Pre-Market-Perpetual">Pre-Market contracts</a></li></ul>|
