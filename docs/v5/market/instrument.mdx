...
|[status](../enum#status) |false |string |Symbol status filter <ul><li>`linear` & `inverse` & `spot `By default returns `Trading` & `PendingOpen` symbols</li><li>`option` By default returns `PreLaunch`, `Trading`, and `Delivering`</li><li>Spot has `Trading` only</li><li>`linear` & `inverse`: when status=PreLaunch, it returns <a href="https://www.bybit.com/help-center/article/Introduction-to-Pre-Market-Perpetual">Pre-Market contracts</a></li></ul>|
|baseCoin |false |string |Base coin, uppercase only <ul><li>Applies to `linear`,`inverse`,`option` **only**</li><li>`option`: returns BTC by default</li><li>Pass `All` to return all option symbols. Only valid when `category=option`</li></ul> |
...
<TabItem value="linear" label="Linear/Inverse" default>
    <table border="0.8">
        ...
        <tr>
            <td>> displayName</td><td>string</td><td>It is the contract name displayed in the Web or App, use utf-8/unicode to parse to store the value</td>
        </tr>
        <tr>
            <td>> fullName</td><td>string</td><td>The full name of the underlying asset, including the ETF name, company name, or cryptocurrency/project name.</td>
        </tr>
        <tr>
            <td>> marketRegion</td><td>string</td><td>Market region of the underlying asset, used to determine its trading calendar and market session hours. <code>US</code>: United States market, <code>CN</code>: Mainland China market, <code>HK</code>:Hong Kong market, <code>KR</code>: South Korea market, <code>""</code>: Not applicable<br/><i>Used for Tradfi Perp</i></td>
        </tr>
        <tr>
            <td>> underlyingTicker</td><td>string</td><td>Underlying ticker/code/symbol of the referenced asset in its native market<br/><i>Used for Tradfi Perp</i></td>
        </tr>
        <tr>
            <td>> forbidUplWithdrawal</td><td>boolean</td><td>Whether to prohibit unrealised profit withdrawal</td>
        </tr>
        <tr>
            <td>> riskParameters</td><td>object</td><td>Risk parameters for limit order price. Note that the <a href="https://announcements.bybit.com/en/article/update-contract-price-limit-enhancement-bltf9ebdcebe3089641/">formula changed</a> in May 2026</td>
        </tr>
        <tr>
