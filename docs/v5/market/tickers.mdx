...
|[symbol](../enum#symbol) |<div class="tooltip">false<span class="tooltiptext">If `category=option`, then `symbol` or `baseCoin` are required</span></div> |string |Symbol name, like `BTCUSDT`, uppercase only |
|baseCoin |<div class="tooltip">false<span class="tooltiptext">If `category=option`, then `symbol` or `baseCoin` are required</span></div> |string |Base coin, uppercase only. Apply to `option` **only**|
