...
|serverTime |long |Server UTC epoch ms. Used to eliminate client local time drift |
|currentPhaseEndTime |long |Current phase end time (epoch ms) |
|nextMarketOpenTime |long |Next main-market open time (epoch ms) |
...
|> startTime |long |Start time (epoch ms) |
|> endTime |long |End time (epoch ms) |
