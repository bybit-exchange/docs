...
sidebar_position: 5
---
Submit a Convert order. Supports both directions: `MINT` (underlying → mStocks Token) and `REDEEM` (mStocks Token → underlying). The client must attach the rate snapshot `frontMultiplier` obtained from [Get Convert List](./convert-list); the server validates it against the currently effective ratio to prevent execution anomalies caused by rate jumps.
