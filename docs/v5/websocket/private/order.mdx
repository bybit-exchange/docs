...
|> avgPrice |string |Average filled price. If unfilled, it is `""` <ul><li>For normal account USDT Perp and Inverse derivatives trades, if a partially filled order, and the final orderStatus is Cancelled, then avgPrice is "0"</li><li>For Classic `spot` is not supported, it is always ""</li></ul> |
