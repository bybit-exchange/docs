...
|> rpiTakerAccess |boolean |Whether the order has matched with an RPI order. `true`: matched with an RPI order, `false`: did not match with an RPI order. |
|> rpiMatchedQty |string |Cumulative RPI matched quantity. |
