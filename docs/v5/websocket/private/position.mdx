...
|> liqPrice | string  |Position liquidation price <ul><li>Normal account & inverse under UTA: it is the real price for isolated and cross positions.</li><li>UTA: it is an estimated price for cross positions(because the unified mode controls the risk rate according to the account)</li></ul>However, this field is empty for Portfolio Margin Mode, and no liquidation price will be provided|
