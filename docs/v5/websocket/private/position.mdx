...
|> netDeltaRatio | string  |Net delta ratio. Option delta is excluded from the calculation. Calculation: `coinAccountSize = abs(longSize + shortSize)`, `coinBaseSize = max(longSize, abs(shortSize))`, `netDeltaRatio = coinAccountSize / coinBaseSize`|
