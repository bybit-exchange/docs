...
**Categorised Topic:** `execution.fast.linear`, `execution.fast.inverse`, `execution.fast.spot`, `execution.fast.option`<br/>
...
|data |array |Object |
|> [category](../../enum#category) | string  | Product type `linear`, `inverse`, `spot`, `option`|
|> symbol |string   |Symbol name |
|> orderId |string |Order ID |
|> isMaker |boolean |`true`: Maker, `false`: Taker |
|> orderLinkId |string |User customized order ID <ul><li>maker trade is always `""`</li><li>If a maker order in the orderbook is converted to taker (by price amend), orderLinkId is also `""`</li><li>For option: maker trade is always `""`, taker trade is always orderLinkId</li></ul> |
