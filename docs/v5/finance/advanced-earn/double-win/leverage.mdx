---
title: Get Double Win Leverage
sidebar_label: Get Double Win Leverage
sidebar_position: 3
---

:::info
* Need authentication. **Up to 1 request** per second per UID.
* Requires Earn permission on the API key.
* This endpoint is **only for RFQ products** (`isRfqProduct=true`). For fixed price range products, use [Get Product Quote](./product-quote) or subscribe to the WebSocket topic [`earn.doublewin.offers`](../websocket/double-win-offer).
:::
