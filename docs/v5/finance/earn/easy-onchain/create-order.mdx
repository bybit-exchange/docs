...
|interestCard|false|Object|Interest bonus card. Only applicable to `FlexibleSaving` `Stake` orders. Obtain `awardId` and `specCode` from [Get Coupon List](./coupons)|
|> awardId|**true**|int64|Interest bonus card unique ID|
|> specCode|**true**|string|Spec code of the interest bonus card|
