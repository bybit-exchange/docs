...
|> status|string|Product stage. See [Product status](#product-status)|
...
`status` is computed dynamically by the server based on the product's time window.

| Value | Condition | Comments |
|:----- |:----------|:----- |
|`NotStarted`|Current time < `subscribeStartAt`|Activity has not started|
|`Online`|`subscribeStartAt` ≤ current time ≤ `subscribeEndAt`|Activity is in progress|
|`Ended`|Current time > `subscribeEndAt`|Activity has ended|
