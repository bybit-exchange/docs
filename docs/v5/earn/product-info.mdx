...
|> bonusEvents|Array |Yesterday's Rewards APR  |
|>> apr|string |Bonus |
|>> coin|string |coin |
...
|> term|int |Only when duration = `Fixed` for OnChain|
...
|> stakeExchangeRate|string |stake exchange rate. Only has value in Onchain LST mode |
|> redeemExchangeRate|string |redeem exchange rate. Only has value in Onchain LST mode|
...
|> stakeTime|string |Staking on-chain time|
|> interestCalculationTime|string |Interest accrual time|
