...
|> side |string  |Position side. `Buy`: long, `Sell`: short <ul><li>Classic account in one-way mode & Unified account (inverse), an empty position returns `None`.</li><li>UTA account(linear contracts), either one-way or hedge mode returns an empty string for an empty position.</li></ul> |
...
|> positionBalance | string  |Position margin <li>UTA(linear): Only meaningful for isolated margin mode</li> |
...
|> cumRealisedPnl | string  |Cumulative realised pnl <ul><li>Futures & Perp: it is the all time cumulative realised P&L</li><li>Option: always 0, meaningless</li></ul>|
