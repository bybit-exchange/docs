...
|> side |string  |Position side. `Buy`: long, `Sell`: short. _Note: For Classic account in one-way mode, an empty position returns `None`. For UTA account, either one-way or hedge mode returns an empty string for an empty position._ |
