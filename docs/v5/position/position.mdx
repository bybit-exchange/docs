...
> **Unified account covers: Linear contract / Options**<br/>
...
|:----- |:-------|:-----|------ |
|[category](../v5/enum#category) |**true** |string |Product type <ul><li>Unified account: `linear`, `option`</li><li>Normal account: `linear`, `inverse`. _Please note that `category` is **not** involved with business logic_</li></ul> |
|symbol |false |string |Symbol name<ul><li>If `symbol` passed, it returns data regardless of having position or not.</li><li>If `symbol`=null, it returns position size grater than zero.</li></ul>|
