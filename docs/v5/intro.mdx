...
Please make sure that your timestamp is in sync with our server time. You can use the [Server Time endpoint](spot/rest-public/server-time#) to check your own machine time.
...
server_time stands for Bybit server time, which can be queried via the [Server Time endpoint](spot/rest-public/server-time#).
:::
