...
* For Read_Write apikey, adding or deleting FiatP2P, FiatBybitPay, FiatBitPay, and FiatConvertBroker is prohibited.
:::
...
|> FiatBybitPay |false |array |Bybit Pay `FaitPayOrder`. **Deprecated**, use `FiatBitPay` instead. During the transition period, passing `FiatBybitPay` remains valid |
...
|> FiatBybitPay |array |Permission of Bybit pay. **Deprecated**, use `FiatBitPay` instead|
