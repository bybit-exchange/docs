...
|> FiatBybitPay |array |Permission of Bybit Pay `FaitPayOrder`. Not applicable to subaccount, always `[]`. **Deprecated**, use `FiatBitPay` instead|
|> FiatBitPay |array |Permission of Bybit Pay `FaitPayOrder`. Not applicable to subaccount, always `[]`. During the transition period, the old field `FiatBybitPay` remains available|
