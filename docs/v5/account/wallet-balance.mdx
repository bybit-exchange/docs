...
|>> spotBorrow |string |Borrow amount by spot margin trade (does not include borrow amount by spot margin active order). This business scenario is not yet online, currently returns `0`. |
