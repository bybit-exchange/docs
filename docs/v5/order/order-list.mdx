...
:::info
* Classic Spot: can get final status orders only
* Classic Spot: market maker can only get recent 3 days order history, please go to web to export
* Classic Spot: retail client can get up to 180 days data
...
|[orderStatus](../enum#orderstatus) |false |string |<ul><li>Classic `spot`: invalid</li><li>Others: return all status orders if not passed</li></ul> |
|startTime |false |integer |The start timestamp (ms). Classic account Spot trading does not support startTime and endTime <ul><li>startTime and endTime are not passed, return 7 days by default</li><li>Only startTime is passed, return range between startTime and startTime+7 days</li><li>Only endTime is passed, return range between endTime-7 days and endTime</li><li>If both are passed, the rule is endTime - startTime <= 7 days</li></ul>|
