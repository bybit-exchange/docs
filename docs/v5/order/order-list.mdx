...
:::tip
* The orders in the last 7 days: UTA(linear,spot,option) supports querying all [closed status](../enum#orderstatus) except "Cancelled" status, UTA(inverse) and Classic account supports querying all status
* UTA(linear,spot,option) for the orders with "Cancelled" (full cancelled order) can query last 24 hours data
...
|baseCoin |false |string |Base coin. _Unified account - inverse & Classic account does **not** support this param_|
|settleCoin |false |string |Settle coin. _Unified account - inverse & Classic account does **not** support this param_|
...
|> avgPrice |string |Average filled price <li>UTA: returns `""` for those orders without avg price</li><li>Classic account: returns `"0"` for those orders without avg price</li>|
