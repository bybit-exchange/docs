...
|orderFilter |false |string |`Order`: active order, `StopOrder`: conditional order for Futures and Spot, `tpslOrder`: spot TP/SL order  <ul><li>Normal `spot`: return `Order` active order by default</li><li>Others: all kinds of orders by default</li></ul>|
...
|> orderType |string |Order type. `Market`,`Limit`. For TP/SL order, it means the order type after triggered <li>`Block trade Roll Back`, `Block trade-Limit`: Unique enum values for Unified account block trades</li>  |
