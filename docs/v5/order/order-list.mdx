...
|orderFilter |false |string |`Order`: active order, `StopOrder`: conditional order for Futures and Spot, `tpslOrder`: spot TP/SL order  <ul><li>Classic `spot`: return `Order` active order by default</li><li>Others: all kinds of orders by default</li></ul>|
|[orderStatus](../enum#orderstatus) |false |string |<ul><li>Classic account `spot`: invalid</li><li>Others: return all status orders if not passed</li></ul> |
