...
:::tip
* The orders in the last 7 days: UTA(linear,spot,option) supports querying all [closed status](../enum#orderstatus), UTA(inverse) and Classic account supports querying all status
* The orders beyond 7 days: supports querying filled orders only
