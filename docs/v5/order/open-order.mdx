...
|symbol |false |string |Symbol name. For `linear`, either `symbol` or `settleCoin` is **required** |
|baseCoin |false |string |Base coin. For `option` **only**. Return all option open orders if not passed|
|settleCoin |false |string |Settle coin. For `linear`, either `symbol` or `settleCoin` is **required** |
