...
sidebar_label: Get Open Orders & Closed Orders (real-time)
sidebar_position: 4
---
...
|openOnly |false |integer |<ul><li>Unified account & Classic account: `0`(default) - query open orders (e.g., New, PartiallyFilled) **only**</li><li>Unified account (spot / linear / option): `1`, Unified account(inverse) & Classic account(linear / inverse): `2`: return a maximum of recent 500 closed status records are kept under each account (e.g., Cancelled, Rejected, Filled orders). <i>If the Bybit service is restarted due to an update, this part of the data will be cleared and accumulated again, but the order records will still be queried in [order history](order-list#)</i></li><li>Classic `spot`: not supported</li></ul>|
