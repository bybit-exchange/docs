...
:::tip
- This endpoint also allows for querying filled, canceled, and rejected orders, but it requires setting either the orderId or orderLinkId. This condition applies only to the most recent 500 orders for spot, linear, and option categories. The inverse category is not subject to this limitation.
...
After a server release or restart, filled, canceled, and rejected orders should only be queried through t[order history](order-list#).
:::
...
|baseCoin |false |string |Base coin <li>Supports `linear`, `inverse` & `option`</li><li>`option`: it returns all option open orders by default</li>|
