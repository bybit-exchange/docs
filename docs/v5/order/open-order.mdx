...
|> [createType](../enum#createtype) |string |Order create type <li>Only for category=linear, inverse or option</li><li>Spot does not have this key</li> |
...
|> cumExecFee |string |<li>`inverse`, `option`: Cumulative executed trading fee.</li><li>`linear`, `spot`: Deprecated. Use `cumFeeDetail` instead.</li> _Classic `spot` is not supported_  |
