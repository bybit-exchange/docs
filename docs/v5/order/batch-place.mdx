...
sidebar_position: 7
---
> **UTA2.0 covers**: Spot, Options, USDT Perpetuals, USDC Contract, Inverse Contract
> **UTA1.0 covers**: Spot, Options, USDT Perpetuals, USDC Contract
...
of the order.
- A maximum of 20 orders (option), 20 orders (inverse), 10 orders (linear), 10 orders (spot) can be placed per request. The returned data list is divided into two lists.
...
:::info
- Check the rate limit instruction when category=`linear`, `spot` or `inverse` [here](../rate-limit#api-rate-limit-rules-for-vipspros)
...
|:----- |:-------|:-----|------ |
|[category](../enum#category) |**true** |string |Product type <li>UTA2.0: `linear`, `option`, `spot`, `inverse`</li><li>UTA1.0: `linear`, `option`, `spot`</li> |
