...
For the minimum unit of the price change, please refer to the `priceFilter` > `tickSize` field in the [instrumentInfo](../market/instrument#).
...
**Option:** a maximum of 100 open orders, institutional client can raise this limit

- **Rate limit:**<br/>
10 req/sec by default for each product type. If you need to raise the rate limit, please contact your client manager or submit an application via <a href="https://www.bybit.com/future-activity/en-US/institutional-services">here</a>
:::
...
|:----- |:-------|:-----|------ |
|category |**true** |string |Product type. `spot`,`linear`,`option`,`option` |
...
|triggerDirection |false |integer |Conditional order param. Used to identify the expected direction of the conditional order. <li>`1`: triggered when market price rises to `triggerPrice`</li><li>`2`: triggered when market price falls to `triggerPrice`</li> |
|orderFilter |false |string |Valid for `spot` ONLY. `Order`,`tpslOrder`. If not passed, `Order` by default|
|triggerPrice |false |string |<li>For future, it is a conditional order param. If you expect the price to rise to trigger your conditional order, make sure:<br/> _triggerPrice > market price_<br/>Else, _triggerPrice < market price_</li><li>For spot, it is a tp/sl order trigger price</li>|
...
|[positionIdx](../enum#positionidx) |**true** |string |Used to identify positions in different position modes. For `linear` ONLY. |
