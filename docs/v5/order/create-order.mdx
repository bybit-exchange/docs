...
|slippageToleranceType |false |string |Slippage tolerance Type for **market order**, `TickSize`, `Percent`<ul><li>Support linear, inverse, spot trading, but take profit, stoploss, conditional orders are not supported</li><li><b>TickSize</b>: <br/>the highest price of Buy order = ask1 x (1 + <code>slippageTolerance</code> x tickSize); <br/>the lowest price of Sell order = bid1 x (1 - <code>slippageTolerance</code> x tickSize)</li><li><b>Percent</b>: <br/>the highest price of Buy order = ask1 x (1 + <code>slippageTolerance</code> x 0.01); <br/>the lowest price of Sell order = bid1 x (1 - `slippageTolerance` x 0.01)</li></ul>|
