...
|bboSideType |false |string |<ul><li>`Queue`: use the order price on the orderbook in the same direction as the `side` </li><li>`Counterparty`: use the order price on the orderbook in the opposite direction as the `side` </li></ul>|
