...
|baseCoin |false|string |Base coin, uppercase only <ul><li>`linear` & `inverse`: If cancel all by baseCoin, it will cancel all corresponding category orders. **Required** if not passing symbol or settleCoin</li></ul>|
