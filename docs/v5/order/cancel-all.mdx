...
|settleCoin |false|string |Settle coin, uppercase only <ul><li>`linear` & `inverse`: **Required** if not passing symbol or baseCoin</li><li>Not support `spot`</li></ul>|
|orderFilter |false |string |<ul><li>category=`spot`, you can pass `Order`, `tpslOrder`, `StopOrder`, `OcoOrder`, `BidirectionalTpslOrder`<br/>If not passed, `Order` by default</li><li>category=`linear` or `inverse`, you can pass `Order`, `StopOrder`<br/>If not passed, all kinds of orders will be cancelled, like active order, conditional order, TP/SL order and trailing stop order</li><li>category=`option`, you can pass `Order`<br/> No matter it is passed or not, always cancel all orders</li></ul>|
...
|success |string |"1": success, "0": fail<li>UTA1.0(inverse), classic(linear, inverse) do not return this field</li>|
