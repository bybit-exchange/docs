...
## Changes on 4 June
* Only main uid can call this api
* a new request param added `product`, and its value can be "DERIVATIVES", "SPOT", "OPTIONS"(default)
* Switch to [Get DCP Info](../account/dcp-info) to get your current DCP configuration
* Once main account is successfully to be configured, all sub accounts DCP will be activated automatically
* Only UTA accounts can trigger DCP on the trading side and are only applicable to USDT/USDC perpetual, USDC Futures, Spot and Options. Classic accounts cannot actually trigger DCP.
