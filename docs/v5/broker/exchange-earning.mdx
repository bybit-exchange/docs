...
* `begin` & `end` are either entered at the same time or not entered
:::
...
|uid |false |string |<ul><li>Input the sub UID to get data of a specific UID</li><li>To get all sub UID's data, leave it empty</li></ul>  |
...
|> bizType |string |Business type. `SPOT`, `DERIVATIVES`, `OPTIONS` |
