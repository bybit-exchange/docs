...
|status |false |integer |Strategy status. `2`: running, `3`/`4`: terminated, `5`: paused, `6`: untriggered |
...
|beginTimeE0 |false |integer |Start time in seconds (Unix timestamp) |
|endTimeE0 |false |integer |End time in seconds (Unix timestamp) |
...
|> status |integer |Strategy status. `2`: running, `3`/`4`: terminated, `5`: paused, `6`: untriggered |
...
|> terminateType |integer |Termination reason code. `0`: none, `1`: user stop, `2`: completed normally, `3`: insufficient balance, and more |
...
<a href="/api-explorer/v5/strategy/strategy-list"><Button>RUN >></Button></a>
...
```http
GET /v5/strategy/list?strategyId=afae1b1b-e765-4348-bb5c-4fb56c2d1e88&status=5&strategyType=twap&pageSize=1&cursor=eyJPZmZzZXQiOjIwMzM3MTk0MTE1MjkxMTc2OTYsIlBhZ2VTaXplIjoxLCJycGNDdXJzb3IiOiIifQ%3D%3D HTTP/1.1
