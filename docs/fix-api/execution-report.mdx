...
| 30040 | [CancelType](../v5/enum#canceltype) | Cancellation reason |
...
| `E` | PendingReplace | Amend in progress. Not delivered in `ExecutionReport` — amend requests are acknowledged via `OrderAmendAck (XAA)`, which serves as confirmation of receipt. The subsequent `ExecutionReport` with `ExecType=5` (Replaced) confirms the final amended state. |
