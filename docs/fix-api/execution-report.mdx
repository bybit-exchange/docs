...
| **Order status push** | Passive order state change | `4`, `C`, etc. |
...
| `E` | PendingReplace | Amend in progress. The amended order state is subsequently delivered via an `ExecutionReport` with `ExecType=0` (New). |
