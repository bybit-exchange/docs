...
| 108 | HeartBtInt | Y | Idle timeout in seconds. If the server receives no message from the client within this window, it sends a `TestRequest (1)` to probe liveness. Any message (not just `Heartbeat`) resets the timer. |
| 553 | Username | **Y** | Your API key |
| 95 | RawDataLength | **Y** | Byte length of `RawData` |
| 96 | RawData | **Y** | Signature (see [Signature](#signature)) |
