...
| 34 | MsgSeqNum | SEQNUM | Both | Y | Monotonically increasing sequence number starting from `1`, **reset on every new session** (see [Sequence Numbers & Gap Handling](#sequence-numbers--gap-handling)) |
| 52 | SendingTime | UTCTIMESTAMP | Both | Y | Send time |

:::caution SenderCompID uniqueness & session lifecycle
...
:::caution Connection quota & rate limits
- A single UID may use **multiple distinct `SenderCompID`s** to open concurrent connections. **Per-UID Per-machine concurrent-connection cap: 20.**
...
- If the server detects a `MsgSeqNum` gap, it **closes the connection**; the client should reconnect and re-Logon starting at `seq=1`.
...
:::info Push behavior with multiple connections
