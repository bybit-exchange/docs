...
| 56 | TargetCompID | STRING | Both | Y | Client sets to `BYBIT`; server sets to the client's `SenderCompID` |
| 34 | MsgSeqNum | SEQNUM | Both | Y | Monotonically increasing sequence number, starting from `1` |
| 52 | SendingTime | UTCTIMESTAMP | Both | Y | UTC send time in `YYYYMMDD-HH:MM:SS.sss` format |
...
|-----|-------|------|----------|-------------|
| 30002 | BapiTimestamp | INT (int64) | N | Unix timestamp in milliseconds. Required for authenticated order requests |
...
| 108 | HeartBtInt | INT | Y | Idle timeout in seconds. If the server receives no message from the client within this window, it sends a `TestRequest (1)` to probe liveness. Any message (not just `Heartbeat`) resets the timer. |
...
| 30023 | Expires | INT | N | API key expiry timestamp (ms), if applicable |
