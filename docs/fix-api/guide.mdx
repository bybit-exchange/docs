...
| `D` | [NewOrderSingle](create-order) | Client → Server | Place a new order |
| `F` | [OrderCancelRequest](cancel-order) | Client → Server | Cancel an existing order |
| `8` | [ExecutionReport](execution-report) | Server → Client | Order ACK, fill reports, and passive status updates |
| `XAR` | [AtomicReplaceRequest](amend-order) | Client → Server | Amend an existing order |
