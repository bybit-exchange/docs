...
- [RFQ APIs and WebSockets](../v5/rfq/basic-workflow) [NEW]
    - We have launched a Bybit RFQ (Request for Quote). Please see our [basic workflow](../v5/rfq/basic-workflow) and the [help centre](https://www.bybit.com/en/help-center/article/Introduction-to-RFQ) for details.
...
- [Rate Limit](../v5/rate-limit) [UPDATE]
    - [Set api rate limit](../v5/rate-limit/API%20Rate%20Limit%20Rules%20for%20PROs/apilimit-set) Change the input parameter `limit` to `rate`, and change the returned field `limit` to `rate`.
    - [Query api rate limit](../v5/rate-limit/API%20Rate%20Limit%20Rules%20for%20PROs/apilimit-query) Change returned field `limit` to `rate`.
- [Rate Limit](../v5/rate-limit) [NEW]
    - Add endpoints to [query ins level api rate limit uages and cap](../v5/rate-limit/API%20Rate%20Limit%20Rules%20for%20PROs/apilimit-query-cap) and [query all uid level api rate limit](../v5/rate-limit/API%20Rate%20Limit%20Rules%20for%20PROs/apilimit-query-all)
