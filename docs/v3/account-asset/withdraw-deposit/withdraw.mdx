---
title: Withdraw
sidebar_label: Withdraw
sidebar_position: 6
slug: /account-asset/withdraw
---

:::info
* Currently we are upgrading the fund account, if your funding account has been upgraded, the API will now be withdrawn via **funding account**, please transfer funds to the funding account first, and then withdraw cash
* If your funding account has not been upgraded, the API will still uses **spot account** to withdraw cash.
* It is expected that in the next two weeks, all users' funding account upgrades will be completed.
* Make sure you have added withdraw address <a href="https://www.bybit.com/user/assets/money-address">here</a>
:::

### HTTP Request
POST `/asset/v3/private/withdraw/create`

### Request Parameters
| Parameter | Required | Type | Comments|
|:----- |:-------|:-----|------ |
|[coin](enum#coin) |<b>true</b> |string |coin name|
|chain|<b>true</b>|string|chain name|
|address|<b>true</b>|string|Withdraw address|
|tag|false|string|Need to fill in if there is a memo/tag when adding the wallet address|
|amount|<b>true</b>|string|withdraw amounts. The minimum withdrawal amount can be obtained from the [Coin Info](coin-info#) Query API|
|timestamp|<b>true</b>|integer|Current timestamp |
|forceChain|false|integer|Force to withdraw on chain or not. <ul><li>`0`：If parsed as internal address, then internal transfer (by default)</li><li>`1`：Force withdraw on chain</li></ul>|

### Response Parameters
| Parameter | Type | Comments|
|:----- |:-----|----- |
|id|string|Withdrawal ID |

### Request Example

```http
POST /asset/v3/private/withdraw/create HTTP/1.1
Host: api-testnet.bybit.com
X-BAPI-SIGN: xxxxxxxxxxx
X-BAPI-API-KEY: xxxxxxxxxxx
X-BAPI-TIMESTAMP: 1671172758674
X-BAPI-RECV-WINDOW: 50000
Content-Type: application/json
Content-Length: 193

{
    "coin": "USDT",
    "chain": "ETH",
    "address": "xxxxxxxxxxx",
    "tag": null,
    "amount": "10",
    "timestamp": 1670919612851,
    "forceChain": 1
}
```

### Response Example

```json
{
    "retCode": 0,
    "retMsg": "success",
    "result": {
        "id": "xxxxxxxxxxx"
      },
    "retExtInfo": {},
    "time": 1671172180550
}
```
