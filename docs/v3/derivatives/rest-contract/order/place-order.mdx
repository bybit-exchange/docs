...
- `option` is not supported for now, it will be available soon.
:::
...
|:----- |:-------|:-----|------ |
|[category](../enum#category) |false |string |Product type. `linear`,`inverse`. Default: `linear`. Cannot place `option` order for now |
