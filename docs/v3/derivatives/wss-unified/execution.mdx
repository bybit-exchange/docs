...
Subscribe execution stream. Will not push funding payments unless they occur on a USDT perpetual, eg BTCUSDT.
...
|>> execType |string |Executed type. `TRADE`,`ADLTRADE`,`FUNDING`,`BUSTTRADE`,`SETTLE`. Only USDT Perpetual has `FUNDING` feed |
