...
|:----- |:-------|:-----|------ |
|[category](../enum#category) |false |string |Product type.`linear`,`inverse`,`option` <ul><li>`category` not passed or =`linear`,`inverse`, then `symbol` is **required**</li><li>`category`=option, then `symbol` is **optional**</li> </ul>|
