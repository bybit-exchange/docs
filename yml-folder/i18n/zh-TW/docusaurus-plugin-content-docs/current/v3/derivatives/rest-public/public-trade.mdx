...
| 參數 | 是否必需 | 類型 | 說明|
|:----- |:-------|:-----|------ |
|[category](../enum#category) |false |string |產品類型.`linear`,`inverse`,`option` <ul><li>當`category`不傳，或者傳的是`linear`,`inverse`, 那麼 `symbol`**必傳**</li><li>當`category`=option, 那麼`symbol` is **可選**</li> </ul>|
