...
|[status](../enum#status) |false |string |Symbol status filter <ul><li>spot has `Trading` only</li><li>`PreLaunch`: when category=linear&status=PreLaunch, it returns pre-market perpetual contract</li></ul>|
