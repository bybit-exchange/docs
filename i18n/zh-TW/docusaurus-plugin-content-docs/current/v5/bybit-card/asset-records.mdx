...
|type |false |string |查詢類型。<br/>`SIDE_QUERY_AUTH`: 授權交易<br/>`SIDE_QUERY_FINANCIAL`: 清算交易。須傳入 `txnId` 和 `orderNo`。<br/>`SIDE_QUERY_REFUND`: 退款交易。須傳入 `txnId` 和 `orderNo`。<br/>以下查詢類型適用於所有用戶：<br/>`SIDE_QUERY_AUTH_ALL`: 所有授權交易<br/>`SIDE_QUERY_AUTH_REVERSAL`: 授權撤銷交易<br/>`SIDE_QUERY_FINANCIAL_ALL`: 所有清算交易<br/>`SIDE_QUERY_FINANCIAL_REFUND`: 清算退款交易|
