...
sidebar_position: 2
---
當前主網環境下, 您仍然可以通過該接口將經典帳戶升級到統一帳戶1.0

下面的解釋說明是針對目前測試網的現狀, 支持將統一帳戶1.0升級到統一帳戶2.0, 同時也是當發佈到主網後尊循的規則:

:::tip 升級引導
通過調用[查詢賬戶配置](account-info)接口來確認當前的帳戶型態

* 當前, 經典帳戶不支持升級到統一帳戶2.0, 它將會在後續打開.

* 如果unifiedMarginStatus=3, 則表示帳戶是統一帳戶1.0, 您需要前往網站或者app端先升級到統一帳戶2.0

* 如果unifiedMarginStatus=4, 則表示帳戶是統一帳戶1.0 Pro, 您可以調用升級接口升級到統一帳戶2.0 Pro. 調用[查詢賬戶配置](account-info)接口,
如果unifiedMarginStatus=6, 則表示帳戶已經成功升級至統一帳戶2.0 Pro.

* 如果unifiedMarginStatus=5, 則表示帳戶是統一帳戶2.0, 您可以調用升級接口升級到統一帳戶2.0 Pro. 調用[查詢賬戶配置](account-info)接口,
如果unifiedMarginStatus=6, 則表示帳戶已經成功升級至統一帳戶2.0 Pro.
:::
...
2. 當從統一帳戶2.0升級到統一帳戶2.0 Pro時, 確保沒有任何掛單<br/><br/>
關於升級統一帳戶1.0 Pro到統一帳戶2.0 Pro的條件, 請確保:
* 對於任意一種訂單類型, 都沒有活動單
