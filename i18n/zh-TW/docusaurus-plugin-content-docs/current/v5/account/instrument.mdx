...
:::info Tradfi
標的資產在其主要市場常規交易時段外，可能會面臨價格波動受限及流動性下降的情形。週末期間，部分合約可能受倉位風控影響，僅支援「只減倉」訂單。
:::
...
<TabItem value="linear" label="Linear/Inverse" default>
    <table border="0.8">
        ...
        <tr>
            <td>> displayName</td><td>string</td><td>網頁或APP端展示的合約名稱，請使用unicode/utf-8來解析或者儲存值</td>
        </tr>
        <tr>
            <td>> fullName</td><td>string</td><td>標的資產的完整名稱，包括 ETF 名稱、公司名稱或加密貨幣／專案名稱。</td>
        </tr>
        <tr>
            <td>> marketRegion</td><td>string</td><td>標的資產所屬的市場區域，用於判斷其交易日曆與市場交易時段。<code>US</code>：美國市場、<code>CN</code>：中國大陸市場、<code>HK</code>：香港市場、<code>KR</code>：韓國市場、<code>""</code>：不適用<br/><i>用於 Tradfi Perp</i></td>
        </tr>
        <tr>
            <td>> underlyingTicker</td><td>string</td><td>標的資產在其原生市場中的股票代號／證券代碼／資產符號<br/><i>用於 Tradfi Perp</i></td>
        </tr>
        <tr>
            <td>> riskParameters</td><td>object</td><td>限價單價格風控參數, 參讀該<a href="https://announcements.bybit.com/en/article/update-contract-price-limit-enhancement-bltf9ebdcebe3089641/">公告內容</a>（2026年5月更新）</td>
        </tr>
        <tr>
        ...
        <tr>
            <td>> isPublicRpi</td><td>boolean</td><td>表示該symbol是否支持RPI報價<ul><li><code>true</code>: 所有用戶均可發送RPI報價</li><li><code>false</code>: 該symbol不支持RPI，所有用戶都無法發送RPI訂單</li></ul><strong>注意:</strong> 值的更新可能有1小時的延遲</td>
        </tr>
        <tr>
            <td>> myRpiPermission </td><td>boolean</td><td><b>廢棄</b>, 值總是和<code>isPublicRpi</code>保持一致</td>
        </tr>
    </table>
</TabItem>
