---
title: 查詢DCP配置
sidebar_label: 查詢DCP配置
sidebar_position: 8.1
---
:::info
當前僅支持Testnet
:::

查詢帳戶的合約(USDT永續, USDC永續和USDC交割) / 現貨 / 期權的dcp配置. 在調用接口前, 請確保已經和客戶經理申請了帳戶DCP配置

* 只有母帳戶支持在後台開通DCP, 一旦開通, 母子所有帳戶都會生效, 且配置信息相同

* 只有配置了的母帳戶能夠從該接口查詢到信息, 沒有配置的母帳戶或者任何子帳戶調用該接口總是返回空

* 在配置時, 若只選擇了部分產品, 比如只開通了期貨, 則現貨和期權是不會返回數據的

* 若配置時提交的是經典帳戶uid, 可以配置成功, 而且可以查詢到信息, 但是交易側的dcp對經典帳戶是無效的, 即經典帳戶在會話斷開時, 無法觸發dcp功能, 但是它下面的UTA子帳戶仍然生效

### HTTP 請求
GET `/v5/account/query-dcp-info`

### 請求參數
無

### 響應參數
| 參數 | 類型 | 說明|
|:----- |:-----|----- |
|dcpInfos |array&lt;object&gt; | |
|> product |string |`SPOT`, `DERIVATIVES`, `OPTIONS`|
|> dcpStatus |string |[設置斷線保護時間](../order/dcp) 開關狀態: `ON`|
|> timeWindow |string |設置的DCP觸發時間窗口. 範圍為[3, 300] 秒, 默認: 10 秒|

### 請求示例
```http
GET /v5/account/query-dcp-info HTTP/1.1
Host: api-testnet.bybit.com
X-BAPI-SIGN: XXXXXX
X-BAPI-API-KEY: XXXXXX
X-BAPI-TIMESTAMP: 1717065530867
X-BAPI-RECV-WINDOW: 5000
```

### 響應示例

```json
// 下面這個實例意味著帳戶只配置了現貨和期貨的dcp功能
{
    "retCode": 0,
    "retMsg": "success",
    "result": {
        "dcpInfos": [
            {
                "product": "SPOT",
                "dcpStatus": "ON",
                "timeWindow": "10"
            },
            {
                "product": "DERIVATIVES",
                "dcpStatus": "ON",
                "timeWindow": "10"
            }
        ]
    },
    "retExtInfo": {},
    "time": 1717065531697
}
```
