...
| 參數 | 是否必需 | 類型 | 說明|
|:----- |:-------|:-----|------ |
|bizType|false |string |業務類型. `SPOT`, `DERIVATIVES`, `OPTIONS`, `CONVERT`, `FIAT_CONVERT` |
...
|>> coin |string |返佣幣種|
|>> earning |string |返佣金額|
|> fiatConvert |array |Object. 法幣閃兌交易的返佣. 如果沒有對應的返佣或者沒有查詢該類型, 則返回空數組 `[]`|
|>> coin |string |返佣幣種|
|>> earning |string |返佣金額|
...
|> userId |string |子帳戶 |
|> bizType |string |業務類型. `SPOT`, `DERIVATIVES`, `OPTIONS`, `CONVERT`, `FIAT_CONVERT` |
|> symbol |string |幣對名 |
|> coin |string |返佣幣種|
|> earning  |string |返佣金額|
