...
|> estimateRedeemTime|string |預計贖回時間。僅適用於 Onchain|
|> estimateStakeTime|string |預計質押時間。僅適用於 Onchain|
|> estimateInterestCalculationTime|string |預計計息時間。僅適用於 Onchain |
|> settlementTime|string |結算時間。僅對 Onchain `Fixed`產品有價值|
