...
|> time|string |時間戳|
|> dto|array |Object: 貸款合約資訊|
|>> apr |string |貸款 APR|
|>> autoRepay|integer |`1`:自動還款; `2`:轉活期; `0`: 不自動還款. 兼容存量訂單;|
|>> canLoan |Boolean |`true`: 可以續貸; `false`: 不可以續貸|
|>> coin|string |幣種|
|>> dueTime |string |到期時間|
|>> id |integer |Id|
|>> interest|string |貸款利息|
|>> loanNo |string |貸款編號|
|>> orderNo|string |訂單編號|
|>> orderTime |string |訂單時間|
|>> period |string |期限|
|>> quantity|string |貸款數量|
|>> remainPenaltyInterest |string |未付罰息|
|>> remainPrincipal|string |未償還本金|
|>> repayPrincipal |string |已還本金|
