...
|symbol |false|string |合約名稱. 對於`linear` & `inverse`: 該字段**必傳**, 若不傳`baseCoin`和`settleCoin` |
|baseCoin |false|string |交易幣種. <ul><li>對於`linear` & `inverse`: 當通過baseCoin來全部撤單時，會將linear和inverse訂單全部撤掉, 且該字段**必傳**, 若不傳`symbol`和`baseCoin`</li><li>對於普通帳戶的現貨: 該字段無效</li></ul> |
