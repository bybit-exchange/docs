...
|symbol |false|string |合約名稱. 該字段**必傳**, 若不傳`baseCoin`和`settleCoin` |
|baseCoin |false|string |交易幣種. 當通過baseCoin來全部撤單時，會將linear和inverse訂單全部撤掉. 該字段**必傳**, 若不傳`symbol`和`baseCoin` |
|settleCoin |false|string |結算幣種. 不支持`spot`. 該字段**必傳**, 若不傳`symbol`和`baseCoin`|
