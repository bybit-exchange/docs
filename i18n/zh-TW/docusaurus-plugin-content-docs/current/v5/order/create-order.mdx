...
|slippageToleranceType |false |string |市價單滑點容差類型, `TickSize`, `Percent`<ul><li>支持現貨和期貨訂單, 但是不支持止盈止損、條件單</li><li><b>TickSize</b>: <br/>限制買單最高價格 = ask1 x (1 + <code>slippageTolerance</code> x tickSize); <br/>限制賣單最低價格 = bid1 x (1 - <code>slippageTolerance</code> x tickSize)</li><li><b>Percent</b>: <br/>限制買單最高價格 = ask1 x (1 + <code>slippageTolerance</code> x 0.01); <br/>限制賣單最低價格 = bid1 x (1 - `slippageTolerance` x 0.01)</li></ul>|
