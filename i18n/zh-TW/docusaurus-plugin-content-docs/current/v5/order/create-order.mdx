...
|bboSideType |false |string |<ul><li>`Queue`: 用orderbook 上與`side`同向的order price掛單 </li><li>`Counterparty`: 用orderbook 上與`side`異向的order price掛單 </li></ul>|
|bboLevel |false |string |`1`,`2`,`3`,`4`,`5`|
