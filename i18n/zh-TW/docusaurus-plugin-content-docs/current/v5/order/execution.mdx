...
|baseCoin |false |string |交易幣種. 統一帳戶(反向)和經典帳戶不支持該字段查詢|
...
|> [createType](../enum#createtype) |string |訂單創建類型<li>統一帳戶: 僅作用於category=linear 或 inverse</li><li>經典帳戶: 總是返回`""`</li><li>現貨、期權不返回該字段</li> |
