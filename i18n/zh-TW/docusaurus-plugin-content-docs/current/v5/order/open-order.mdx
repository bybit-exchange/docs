...
|orderFilter |false |string|`Order`: 活動單, `StopOrder`: 條件單，僅期貨有效, `tpslOrder`: 止盈止損單, 僅現貨有效 <ul><li>普通帳戶現貨: 默認是`Order`活動單</li><li>其他: 默認是全部類型訂單|
...
|> blockTradeId |string |Paradigm大宗交易Id. _普通帳戶現貨交易不支持_ |
|> symbol |string |合約名稱 |
|> price |string |訂單價格 |
|> qty |string |訂單數量 |
