...
|orderFilter |false |string|`Order`: 活動單, `StopOrder`: 條件單, 支持現貨和期貨, `tpslOrder`: 止盈止損單, 僅現貨有效, `OcoOrder`: OCO訂單, 僅UTA現貨有效 <ul><li>普通帳戶現貨: 默認是`Order`活動單</li><li>其他: 默認是全部類型訂單</li></ul>|
