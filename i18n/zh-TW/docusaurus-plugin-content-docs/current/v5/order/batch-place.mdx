...
- 每個請求包含的訂單數最大是: 20筆(反向合約), 20筆(期权), 10筆(正向合約), 10筆(現貨)，返回的數據列表中分成兩個list，訂單創建的列表和創建結果的信息返回，兩個list的訂單的序列是完全保持一致的。
:::

:::info
- 當category=linear, inverse 或者 spot時, 請從[這裡](../rate-limit#不同vippro等級的接口限頻規則)查閱其API限頻說明
...
| 參數 | 是否必需 | 類型 | 說明|
|:----- |:-------|:-----|------ |
|[category](../enum#category) |**true** |string |產品類型<li>統一帳戶2.0: `linear`, `option`, `spot`, `inverse`</li><li>統一帳戶1.0: `linear`, `option`, `spot`</li> |
...
|> [tpTriggerBy](../enum#triggerby) |false |string |觸發止盈的價格類型. `MarkPrice`, `IndexPrice`, 默認:`LastPrice`<br/>`linear`, `inverse`有效|
|> [slTriggerBy](../enum#triggerby) |false |string |觸發止損的價格類型. `MarkPrice`, `IndexPrice`, 默認:`LastPrice`<br/>`linear`, `inverse`有效|
...
|> tpOrderType |false |string |止盈觸發後的訂單類型 <ul><li>`linear`, `inverse``: `Market`(默認), `Limit`. 對於tpslMode=Full, 僅支持tpOrderType=Market</li><li>`spot`(UTA): <br/>`Market`: 當帶了參數"takeProfit", `Limit`: 當帶了參數"takeProfit" 和 "tpLimitPrice" </li></ul>|
|> slOrderType |false |string |止損觸發後的訂單類型 <ul><li>`linear`, `inverse``: `Market`(默認), `Limit`. 對於tpslMode=Full, 僅支持slOrderType=Market</li><li>`spot`(UTA): <br/>`Market`: 當帶了參數"stopLoss", `Limit`: 當帶了參數"stopLoss" 和 "slLimitPrice" </li></ul>|
