...
|orderFilter |false |string |`Order`: 普通單, `StopOrder`: 條件單, 支持現貨和期貨, `tpslOrder`: 現貨止盈止損單 <ul><li>普通帳戶現貨: 默認是`Order`活動單</li><li>其他: 默認是全部類型訂單</li></ul>|
...
|> orderType |string |訂單類型. `Market`,`Limit`. 對於止盈止損單, 則表示為觸發後的訂單類型 <li>`Block trade Roll Back`, `Block trade-Limit`: 統一帳戶大宗交易獨有的兩個枚舉值</li> |
