...
| 參數 | 是否必需 | 類型 | 說明|
|:----- |:-------|:-----|------ |
|[category](../enum#category) |**true** |string |產品類型 <ul><li>統一帳戶: `spot`, `linear`, `option`</li><li>普通帳戶: `linear`, `inverse`. 這裡`category`字段不參與業務邏輯，僅做路由使用</li></ul> |
|symbol |false |string |合約名稱|
|baseCoin |false |string |交易幣種. 普通帳戶不支持該字段的查詢 |
|orderId |false |string |訂單ID |
|orderLinkId |false |string |用戶自定義訂單ID |
|orderFilter |false |string |`Order`: 普通單, `StopOrder`: 條件單, `tpslOrder`: 現貨止盈止損單. 默認: 全部類型訂單|
