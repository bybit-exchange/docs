...
|symbol |false |string |合約名稱<li>若傳了`symbol`, 則不管是否有倉位都返回該symbol數據</li><li>若`symbol`不傳, 則僅返回有實際倉位的數據</li><li>若category和symbol不匹配, 則不會返回有效的倉位數據</li>|
...
|> sessionAvgPrice | string  | 結算價格, 僅對 USDC 永續有效 |
