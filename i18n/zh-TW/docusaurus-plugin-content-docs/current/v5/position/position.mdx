...
| 參數 | 是否必需 | 類型 | 說明|
|:----- |:-------|:-----|------ |
|[category](../v5/enum#category) |**true** |string |產品類型 <ul><li>統一帳戶: `linear`, `option`</li><li>普通帳戶: `linear`, `inverse`. 這裡`category`字段不參與業務邏輯，僅做路由使用</li></ul> |
|symbol |false |string |合約名稱<ul><li>若傳了`symbol`, 則不管是否有倉位都返回該symbol數據</li><li>若`symbol`不傳, 則僅返回有實際倉位的數據</li></ul>|
...
|> autoAddMargin | integer |是否自動追加保證金. `0`: 否, `1`: 是. _普通帳戶的獨有字段_ |
