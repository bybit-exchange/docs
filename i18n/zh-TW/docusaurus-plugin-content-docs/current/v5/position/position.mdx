...
|> riskId | integer | 风险限额ID，參見[風險限額](../v5/market/risk-limit#)接口. _注意：若賬戶為組合保證金模式(PM)，該字段返回0，riskId規則失效_|
|> riskLimitValue | string | 當前風險限額ID對應的持倉限制量. _注意：若賬戶為組合保證金模式(PM)，該字段返回0，riskId規則失效_|
|> symbol | string | 合約名称 |
