...
查詢USDC交割和期權的交割紀錄, 返回結果按照`deliveryTime`降序排列
> 適用於: 統一帳戶1.0, 統一帳戶2.0
...
| 參數 | 是否必需 | 類型 | 說明|
|:----- |:-------|:-----|------ |
|[category](../enum#category) |**true** |string |產品類型<li>統一帳戶2.0: `inverse`(反向交割), `linear`(USDC交割), `option`</li><li>統一帳戶1.0: `linear`(USDC交割), `option`</li> |
