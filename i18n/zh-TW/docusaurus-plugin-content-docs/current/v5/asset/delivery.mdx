...
查詢反向交割 / USDC交割 / 期權的交割紀錄, 返回結果按照`deliveryTime`降序排列
...
| 參數 | 是否必需 | 類型 | 說明|
|:----- |:-------|:-----|------ |
|[category](../enum#category) |**true** |string |產品類型<li>[統一帳戶2.0](../acct-mode#統一帳戶20): `inverse`(反向交割), `linear`(USDC交割), `option`</li><li>[統一帳戶1.0](../acct-mode#統一帳戶10): `linear`(USDC交割), `option`</li> |
