...
|fromMemberId |false |string |劃出UID|
|toMemberId |false |string |劃入UID|
...
|> withdrawReqId |string |扣錢流水id（內部使用）， 可以用於跟[查詢交易日誌](../../account/transaction-log)裡的`id`做映射 |
|> depositReqId |string |加錢流水id（內部使用）， 可以用於跟[查詢交易日誌](../../account/transaction-log)裡的`id`做映射 |
