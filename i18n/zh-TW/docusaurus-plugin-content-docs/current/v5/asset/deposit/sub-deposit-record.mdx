...
|> depositType|string|入金類型. `0`: 正常充值, `10`: 充值觸發每日限額, `20`: 觸發AML充值，請前往網頁端提交材料，`50`: 帳戶存在限制，無法正常上帳，請前往網頁端提走資金|
|> fromAddress|string|入金來源地址, 僅當入金來自鏈上且來源地址唯一時返回地址, 其餘則返回`""`|
|> taxDepositRecordsId|string|Bybit EU（奧地利）用戶用於稅務目的, 保稅記錄id |
|> taxStatus|integer|Bybit EU（奧地利）用戶用於稅務目的 <ul><li>0: No reporting required</li><li>1: Reporting pending</li><li>2: Reporting completed</li></ul>|
|> travelRuleStatus |integer|Travel Rule 審核狀態，非合規區可以忽略該字段 <ul><li>`0`: Approved — 已通過，可繼續後續流程</li><li>`1`: CollectInfo — 需補充對手方信息，請重新提交問卷</li><li>`2`: Pending — 審核中，請輪詢入金查詢接口</li><li>`3`: Rejected — 已拒絕或失敗（含取消）</li></ul> |
