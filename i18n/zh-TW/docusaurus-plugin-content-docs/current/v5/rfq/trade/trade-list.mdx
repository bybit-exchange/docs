...
獲取交易資訊。**每秒最多 50 次請求**  
:::info
<li>字段查詢優先級：rfqId > rfqLinkId > quoteId > quoteLinkId</li>
:::
...
|traderType |**false** |string |交易者類型，`quote` 或 `request`，默認為 `quote`<li>`Request`：詢價方，查詢自己詢價單的成功交易</li><li>`Quote`：報價方，查詢自己報價單的成功交易</li>|
...
|>> quoteDestCode |string |報價方的唯一識別代碼，若報價時設置匿名為 `true` 則不可見|
