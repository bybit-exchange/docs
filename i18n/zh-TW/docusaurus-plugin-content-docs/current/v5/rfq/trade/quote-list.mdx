...
:::info
<li>獲取使用者發送或接收的報價資訊，從資料庫查詢，有延遲</li>
...
|traderType |**false**  |string |交易者類型，`quote` 或 `request`，默認為 `quote`<li>`Request`：詢價方，查詢自己接收到的報價</li><li>`Quote`：報價方，查詢自己發布的報價</li>|
|status |string |詢價單狀態：`Active`、`Canceled`、`PendingFill`、`Filled`、`Expired`、`Failed` |
