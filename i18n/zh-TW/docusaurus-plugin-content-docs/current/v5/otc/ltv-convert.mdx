...
|> ltv | string	| 風險率 <li>該數據是實時計算</li><li>如果持有機構借貸, 強烈建議每秒查詢一次ltv。當達到0.9 (90%)時即觸發強平</li>. 當 `liqStatus` != 0 時，傳回空字串。|
...
|> balance | string	| 總資產(保證金幣種資產折算為USDT資產). 可以參考<a href="https://www.bybit.com/zh-MY/help-center/s/article/Over-the-counter-OTC-Lending">這裡</a>了解詳細計算。當 `liqStatus` != 0 時，傳回空字串。 |
