...
<table>
    ...
    <tr>
        <td><a href="order/order-list#">查詢歷史訂單</a></td><td>1. <code>orderStatus</code>不傳, 默認查詢所有終態訂單<br/>2. 支援參數<code>baseCoin</code>和<code>settleCoin</code><br/>3. 不支持活動單查詢, 部分終態訂單有限查詢<br/>4. 只能查詢升級後產生的訂單</td><td>1. <code>orderStatus</code>不傳, 默認查詢活動態和終態訂單<br/>2. 不支持參數<code>baseCoin</code>和<code>settleCoin</code><br/>3. 總是支持活動單和各種終態單查詢</td>
    </tr>
    <tr>
    ...
    <tr>
        <td rowSpan="5" align="center">持倉</td><td><a href="position#">查詢持倉(實時)</a></td><td>1. 不支持symbol傳多個<br/>2. 響應字段中, "tradeMode", "autoAddMargin", "liqPrice", "bustPrice" 使用有變化</td><td>1. 支持symbol傳多個</td>
    </tr>
    <tr>
    ...
    <tr>
        <td rowSpan="1" align="center">WebSocket流/交易</td><td><a href="websocket/trade/guideline#">Websocket下單指南</a></td><td>支持反向合約</td><td>不支持反向合約</td>
    </tr>
</table>
