...
| 參數 | 是否必需 | 類型 | 說明|
|:----- |:-------|:-----|------ |
|category | **true**|integer |`2`: 貴金屬（黃金、白銀）合約協議<br/>`3`: 原油合約協議<br/>_該字段仍然可用，但後續新增的枚舉值將不再添加至此字段，請使用 `categoryV2`。_|
|categoryV2 |false |integer |`2`: 貴金屬（黃金、白銀）合約協議<br/>`3`: 原油合約協議|
