...
:::note 版本相容性說明
在 version 2 中，我們新增了一個 boolean 型別的請求參數。但對於某些程式語言，若未在請求 payload 中明確包含該欄位，可能會導致問題。
目前建議繼續使用 `version=1` 發送請求。在回應端，您可能仍會看到 `version=2`，這是目前的內部測試版本，尚未正式發布。
目前的 `version=1` 已完全穩定，可用於生產環境。
:::

## 主網
預計將於2026年6月初發布. 測試網URL: `wss://stream-testnet.bybits.org/v5/trade-sbe`
