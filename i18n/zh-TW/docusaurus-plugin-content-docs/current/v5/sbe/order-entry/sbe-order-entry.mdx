...
sidebar_label: SBE Order Entry 接入指南
sidebar_position: 2
---
...
| 環境 | URL |
|------|-----|
| 正式環境 (Mainnet) | `wss://xxxxxxx.aws.bybit.com` |
| 測試環境 (Testnet) | `wss://stream-testnet.bybit.com/v5/sbe/trade` |
...
|-------------|------|-----------|-------------|
| 1  | `AuthReq`                | Client → Server | 驗證請求 Authentication request |
| 2  | `AuthResp`               | Server → Client | 驗證回應 Authentication response |
| 3  | `PingReq`                | Client → Server | 心跳 ping Heartbeat ping |
| 4  | `PongResp`               | Server → Client | 心跳 pong Heartbeat pong |
| 5  | `CreateOrderReqV5`       | Client → Server | 建立單筆訂單 Create single order |
| 6  | `CreateOrderRespV5`      | Server → Client | 建立訂單回應 Create order response |
| 7  | `ReplaceOrderReqV5`      | Client → Server | 修改單筆訂單 Replace single order |
| 8  | `ReplaceOrderRespV5`     | Server → Client | 修改訂單回應 Replace order response |
| 9  | `CancelOrderReqV5`       | Client → Server | 取消單筆訂單 Cancel single order |
| 10 | `CancelOrderRespV5`      | Server → Client | 取消訂單回應 Cancel order response |
| 11 | `BatchCreateOrderReqV5`  | Client → Server | 批次建立訂單 Batch create orders |
| 12 | `BatchCreateOrderRespV5` | Server → Client | 批次建立回應 Batch create response |
| 13 | `BatchReplaceOrderReqV5` | Client → Server | 批次修改訂單 Batch replace orders |
| 14 | `BatchReplaceOrderRespV5`| Server → Client | 批次修改回應 Batch replace response |
| 15 | `BatchCancelOrderReqV5`  | Client → Server | 批次取消訂單 Batch cancel orders |
| 16 | `BatchCancelOrderRespV5` | Server → Client | 批次取消回應 Batch cancel response |
| 17 | `CommonErrResp`          | Server → Client | 通用錯誤回應 Generic error response |
