...
|> extraFees |List<Json> |交易費率。目前，僅針對kyc=Indian用戶或在印尼網站的現貨訂單或在歐盟站的現貨法定貨幣訂單返回此數據。在其他情況下，傳回空字串。字段枚舉: [feeType](../../enum#extrafeesfeetype), [subFeeType](../../enum#extrafeessubfeetype)
