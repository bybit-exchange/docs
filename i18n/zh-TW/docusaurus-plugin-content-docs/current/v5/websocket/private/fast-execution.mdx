...
|data |array |Object |
|> [category](../../enum#category) | string | 產品類型 `linear`, `inverse`, `spot`, `option`|
|> symbol |string |合約名稱|
|> orderId |string |訂單ID |
|> isMaker |boolean |`true`: maker成交, `false`: taker成交 |
|> orderLinkId |string |用戶自定義訂單ID <ul><li>maker成交總是返回`""`</li><li>當maker訂單在訂單簿中轉化成了taker單(比如修改了價格), 這種情況orderLinkId也是`""`</li><li>期權: maker成交永遠返回`""`, taker成交永遠返回orderLinkId</li></ul>|
