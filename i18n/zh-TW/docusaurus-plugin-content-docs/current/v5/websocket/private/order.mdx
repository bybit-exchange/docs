...
|> cumExecFee |string |已棄用. 訂單累計成交的手續費 <ul><li>經典帳戶`spot`: 表示的是最近一次成交的手續費.</li><li>升級到統一帳戶後, 您可以使用[成交](execution)頻道中的`execFee`字段來獲取每次成交的手續費</li></ul> |
...
|> cumFeeDetail |json |累計交易費用詳情，替换`cumExecFee`和`feeCurrency`
