...
|data |array |Object |
|> category |string |產品類型 <ul><li>統一帳戶: 沒有該字段</li><li>統一帳戶(反向)和普通帳戶: `linear`, `inverse`.</li></ul>|
|> symbol |string |合約名稱 |
|> side |string |持倉方向. `Buy`,`Sell` |
|> size |string |持倉大小 |
|> [positionIdx](../../enum#positionidx) |integer |倉位標識|
|> tradeMode |integer |交易模式. `0`: 全倉, `1`: 逐倉. 統一帳戶(現貨/正向合約/期權)下總是`0` |
...
|> positionBalance |string |倉位保證金. _統一帳戶(現貨/正向合約/期權)沒有該字段_|
|> autoAddMargin |integer |是否自動追加保證金. `0`: false, `1`: true. _統一帳戶(現貨/正向合約/期權)沒有該字段_|
|> positionMM |string |倉位維持保證金. _注意: 組合保證金模式下，該字段返回空字符串_|
|> positionIM |string |倉位初始保證金. _注意: 組合保證金模式下，該字段返回空字符串_|
|> liqPrice |string |預估強平價. 統一帳戶(現貨/正向合約/期權)為空 |
|> bustPrice |string |預估破產價. 統一帳戶(現貨/正向合約/期權)為空 |
