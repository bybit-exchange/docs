...
sidebar_position: 3
---
根據訂單號或客戶端冪等鍵查詢單筆股票買賣訂單的完整狀態、參數與成交明細。用於 MM 側對賬、狀態輪詢與失敗原因定位。
...
| 參數 | 是否必需 | 類型 | 說明|
|:----- |:-------|:-----|------ |
|orderNo |false |string |系統訂單號。`orderNo` 與 `requestId` 二選一 |
|requestId |false |string |客戶端冪等鍵。`orderNo` 與 `requestId` 二選一 |

### 響應參數
| 參數 | 類型 | 說明|
|:----- |:-----|----- |
|orderNo |string |系統訂單號 |
|requestId |string |客戶端冪等鍵（若下單時提供） |
|symbol |string |股票代碼 |
