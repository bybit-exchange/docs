...
|serverTime |long |服務端 UTC epoch ms，用來消除用戶本地時間誤差 |
|currentPhaseEndTime |long |當前時段結束時間（epoch ms） |
|nextMarketOpenTime |long |下次主市場開盤時間（epoch ms） |
|stocksTradingHoliday |array&lt;object&gt; |所有不可交易時間段 |
|> [type](../enum#stocks-tradingholidaytype) |string |類型。`EARLY_CLOSE`, `HOLIDAY` |
|> startTime |long |開始時間（epoch ms） |
|> endTime |long |結束時間（epoch ms） |
