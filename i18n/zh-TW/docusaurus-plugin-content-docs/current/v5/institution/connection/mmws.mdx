...
MMWS 的設計重點是連線穩定性。它可以減少非預期斷線、降低推送延遲發生率，並避免因頻繁重連而導致的握手失敗。不過，MMWS 並不保證相較標準公共 WebSocket endpoint 具備絕對延遲優勢。

## 支援路徑

MMWS 支援以下 V5 WebSocket 路徑:
...
3. 將公共 WebSocket hostname 替換為 Bybit 提供的專屬 MMWS hostname。
4. 保持與標準 V5 WebSocket API 相同的鑑權、訂閱、ping/pong、重連和消息解析邏輯。
