...
1. **合約盈虧回撤 ADL (基於分組保險池的新機制, 詳見用例 1, 2)**  
   - **觸發條件**:  
     `balance` (保險基金餘額) > `adlTriggerThreshold` (合約盈虧回撤 ADL 的觸發閾值, 配置參數, 正值)  
     且 `pnlRatio` < `insurancePnlRatio` (觸發 ADL 的盈虧比例閾值, 配置參數, 負值)  

     其中:  
     - **pnlRatio**: symbol 在 8 小時內的回撤比例  
       計算公式: `pnlRatio = (8h 最大 PnL - 當前 PnL) / 保險池 8h 最大餘額`  
       *8h 最大 PnL 與 當前 PnL 的具體數值未在 API 中直接提供*  
     - **保險池 8h 最大餘額**: 最近 8 小時內, 該分組保險池的最大餘額  

   - **停止條件**:  
     `pnlRatio` > `adlStopRatio` (ADL 停止回撤比例閾值, 配置參數, 負值)  

2. **保險池整體淨值 (equity) 虧損觸發 ADL (舊機制, 詳見用例 3)**  
   - **觸發條件**:  
     `balance` (保險基金餘額) ≤ 0  
   - **停止條件**:  
     `balance` (保險基金餘額) > 0  
...
3. **`symbol` 在分組保險池中出現過於劇烈的保證金虧損 (可視作保險池整體淨值虧損的一種特殊情況)**  
   ...
   - **觸發條件**:  
     當虧損的 `symbol` 被移出所屬保險池, 導致 `balance` (保險基金餘額) ≤ 0 時, 觸發 ADL  
   - **停止條件**:  
     `balance` (保險基金餘額) > 0  
:::
...
2. **場景 2: 保險池在 8 小時內產生大額盈利, `symbol` 虧損觸發 ADL**  
