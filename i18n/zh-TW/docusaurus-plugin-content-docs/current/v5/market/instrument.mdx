...
|[status](../enum#status) |false |string |交易對狀態篩選 <ul><li>`linear` & `inverse` & `spot ` 默認返回`Trading`和`PendingOpen`狀態的交易對</li><li>`option` 默認返回 `PreLaunch`, `Trading`, `Delivering`狀態的交易對</li><li>現貨 僅有`Trading`狀態</li><li>`linear` & `inverse`: status=PreLaunch可以查詢到盤前交易的合約</li></ul>|
|baseCoin |false |string |交易貨幣. 僅對期貨/期權有效 <ul><li>對於期權, 默認返回baseCoin為BTC的</li><li>傳入 `All` 可返回所有期權的交易對，僅 `category=option` 時有效</li></ul>|
