...
| 56 | TargetCompID | STRING | 雙向 | Y | 用戶端設為 `BYBIT`；伺服器設為用戶端的 `SenderCompID` |
...
| 108 | HeartBtInt | INT | **Y** | 空閒逾時閾值（秒）。若伺服器在此時間內未收到用戶端任何訊息，將發送 `TestRequest (1)` 探測連線存活。任何訊息（不限於 `Heartbeat`）均會重置此計時器。|
...
| 30023 | Expires | INT | N | API 金鑰到期時間戳（ms，如適用）|

**回應欄位（伺服器 → 用戶端）：**

| Tag | 欄位 | 類型 | 說明 |
|-----|-------|------|-------------|
| 30003 | ConnId | STRING | 伺服器分配的唯一連線識別碼。聯絡客服排查問題時請提供此值，後續訊息中無需回傳。|
