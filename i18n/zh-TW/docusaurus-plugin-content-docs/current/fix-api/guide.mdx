...
Bybit FIX API 透過持久 TLS 連線，使用 **FIX 4.4** 協議提供現貨交易的低延遲存取。
...
| 環境 | Host | Port |
|---|---|---|
| 正式環境 (9月1日) | `fix-oe.bybit.com` | `9000` |
...
| 34 | MsgSeqNum | SEQNUM | 雙向 | Y | 單調遞增序列號，從 `1` 開始，**每個新 session 重置**（詳見 [序列號與 Gap 處理](#序列號與-gap-處理)）|
...
:::caution 連線數與速率限制
- 同一 UID 可使用**多個不同的 `SenderCompID`** 建立多條並發連線。同一 UID 同一台机器 ** 連線數上限：20 條**
...
- **不支援斷線續連（session resume）**。連線斷開時尚未消費的訊息**不會**在重連時補發。
