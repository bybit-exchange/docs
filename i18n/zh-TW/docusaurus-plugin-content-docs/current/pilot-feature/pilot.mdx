...
---
## 增加WS私有連接可配置斷線時間
* 可用日期: *測試網 - 已上線, 主網 - 2023年8月17日*
* 覆蓋範圍: *構建v5私有連接時可用該配置參數*
* Details: *構建連接時, 設置`/v5/private?max_active_time=`, 值可以是從`30s`到`600s`之間。當服務端與客戶端之間無上下行數據時, 當達到這個配置的時間間隔時, 服務端則會斷開連接 (並不完全準確, 因為當前ticker的輪訓時間是30秒)*
