...
| 參數 | 是否必須 | 類型 | 說明|
|:----- |:-------|:-----|------ |
|withdrawID |false |int64 |提現ID。 (提現API響應參數中的提現請求ID) |
...
|status|int|[提現狀態](../enum#withdrawStatus)|
...
|withdrawId|string|提現ID。 (提現API響應參數中的提現請求ID)|
|withdrawType|int|出金類型.<ul><li>`0`：鏈上出金;</li> <li>`1`：內部地址轉帳</li></ul>|
