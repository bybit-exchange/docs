...
| 參數 | 是否必須 | 類型 | 說明|
|:----- |:-------|:-----|------ |
|subMemberId |<b>true</b> |integer | 子帳戶userId
|startTime |false |int |開始時間 (精確到毫秒)。 默認為當前時間之前30天 |
|endTime |false |int |結束時間 (精確到毫秒)。 默認為當前時間 |
|[coin](enum#coin) |false |string |幣種名：舉例，BTC。默認全部 |
|cursor |false |string |游標信息：用來分頁。 默認空|
|limit |false |int |每頁條數。[`1`, `50`], 默認為50 |
