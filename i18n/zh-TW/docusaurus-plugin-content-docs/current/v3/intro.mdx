...
- **衍生品 - 統一保證金:** 您能夠通過`unified/v3`接口和V5接口，交易_USDT永續_, _USDC永續_, _期權_ with `unified/v3` endpoints。
- **衍生品 - 合約:** 您可以通過`contract/v3`接口交易_USDT永續_, _反向永續_, _反向交割_。對於_USDC永續_, _期權_, 將會很快全面支持。
- **現貨:** 您可以通過`spot/v3`交易現貨，不管您的帳戶模式是統一保證金帳戶還是普通帳戶。
- **跟單交易:** 您必須首先成為**交易員**才能使用該套接口。
- **帳戶資產:** 涉及了劃轉、提現、密鑰相關等接口。
- **稅:** 您可以通過這部分接口生成稅相關的報告。
