...
|tpLimitPrice |false |string |**(僅testnet)** 觸發止盈後轉換為限價單的價格. 當且僅當原始訂單下單時創建的是部分止盈止損限價單, 本字段才有效|
|slLimitPrice |false |string |**(僅testnet)** 觸發止損後轉換為限價單的價格. 當且僅當原始訂單下單時創建的是部分止盈止損限價單, 本字段才有效|
